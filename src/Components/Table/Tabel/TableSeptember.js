...
export default function BasicTable() {
  ...
  function getDataSeptember(){
    axios.get('http://127.0.0.1:8000/api/septemberTabel').then((response) => {
        setData(response.data);
        console.log(response.data)
    })
  }

  function hapusSeptember(id){
    console.log(id)
    swal({
    ...
    axios.delete('http://127.0.0.1:8000/api/septemberTabel/delete/'+id).then((response) => {
            getDataSeptember()
          ...
          {data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map( (row) => (
            <TableRow  
              ...
              <Grid container>
                ...
                <Grid container item>
                    <Button startIcon={<CreditCardIcon />} variant="contained" fullWidth color="inherit" href={"/EditSeptember/"+ row.id_tableSep}>
                      Edit
                    </Button>
                </Grid>
                  
                  <Grid container item>
                    <Button variant="contained" fullWidth color="error" onClick={() => hapusSeptember(row.id_tableSep)}>
                      Delete
                    </Button>
                  </Grid>
