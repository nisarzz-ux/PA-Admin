...
export default function BasicTable() {
  ...
  function getDataDesember(){
    axios.get('http://127.0.0.1:8000/api/desemberTabel').then((response) => {
        setData(response.data);
        console.log(response.data)
    })
  }

  function hapusDesember(id){
    console.log(id)
    swal({
    ...
    axios.delete('http://127.0.0.1:8000/api/desemberTabel/delete/'+id).then((response) => {
            getDataDesember()
          ...
          {data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
            <TableRow  
              ...
              <Grid container>
                ...
                <Grid container item>
                    <Button startIcon={<CreditCardIcon />} variant="contained" fullWidth color="inherit" href={"/EditDesember/"+ row.id_tableDes}>
                      Edit
                    </Button>
                </Grid>
                  
                  <Grid container item>
                    <Button startIcon={<DeleteIcon />} variant="contained"  fullWidth color="error" onClick={() => hapusDesember(row.id_tableDes)}>
                      Delete
                    </Button>
                  </Grid>
