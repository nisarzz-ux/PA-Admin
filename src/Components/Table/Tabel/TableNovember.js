...
export default function BasicTable() {
  ...
  function getDataNovember(){
    axios.get('http://127.0.0.1:8000/api/novemberTabel').then((response) => {
        setData(response.data);
        console.log(response.data)
    })
  }

  function hapusNovember(id){
    console.log(id)
    swal({
    ...
    axios.delete('http://127.0.0.1:8000/api/novemberTabel/delete/'+id).then((response) => {
            getDataNovember()
          ...
          {data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
            <TableRow  
              ...
              <Grid container>
                ...
                <Grid container item>
                    <Button startIcon={<CreditCardIcon />} variant="contained" fullWidth color="inherit" href={"/EditNovember/"+ row.id_tableNov}>
                      Edit
                    </Button>
                </Grid>
                  
                  <Grid container item>
                    <Button startIcon={<DeleteIcon />} variant="contained"  fullWidth color="error" onClick={() => hapusNovember(row.id_tableNov)}>
                      Delete
                    </Button>
                  </Grid>
